...
get_area_mean_test() ->
  ...
  M  = pollution:add_station("S1", {0,0}, M0),
  M1 = pollution:add_station("S2", {0,1}, M),   % distance 1.0 from {0,0}
  M2 = pollution:add_station("S3", {0,3}, M1),  % distance 3.0 from {0,0}
  M3 = pollution:add_value("S1", {{2023,1,1},{0,0,0}}, "PM10", 10, M2),
  M4 = pollution:add_value("S2", {{2023,1,1},{0,0,0}}, "PM10", 20, M3),
  M5 = pollution:add_value("S3", {{2023,1,1},{0,0,0}}, "PM10", 30, M4),

  ?assertMatch(10.0, pollution:get_area_mean("PM10", {0,0}, 0.5, M5)),  % tylko S1
  ?assertMatch(15.0, pollution:get_area_mean("PM10", {0,0}, 1.5, M5)),  % S1 + S2
  ?assertMatch(20.0, pollution:get_area_mean("PM10", {0,0}, 4.0, M5)).  % S1 + S2 + S3

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_area_mean_fail_test() ->
  M0 = pollution:create_monitor(),
  M  = pollution:add_station("S1", {0,0}, M0),
  M1 = pollution:add_value("S1", {{2023,1,1},{0,0,0}}, "PM10", 10, M),

  ?assertMatch({error, _}, pollution:get_area_mean("PM10", {5,5}, 1.0, M1)),  % brak stacji w obszarze
  ?assertMatch({error, _}, pollution:get_area_mean("PM25", {0,0}, 1.0, M1)).  % brak odczytow tego typu
