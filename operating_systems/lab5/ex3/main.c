...
#define SHM_NAME "/myshm3"
#define SEM_ANY "/any3"
#define SEM_FULL_PRIORITY "/full_priority3"
#define SEM_EMPTY_PRIORITY "/empty_priority3"
#define SEM_MUTEX_PRIORITY "/mutex_priority3"
#define SEM_FULL_NORMAL "/full_normal3"
#define SEM_EMPTY_NORMAL "/empty_normal3"
#define SEM_MUTEX_NORMAL "/mutex_normal3"
#define SEM_ORDER "/order3_%d"
...
void manager(void) {
  ...
  while (1) {
    ...
    int np, nn;
    sem_getvalue(sem_full_priority, &np);
    sem_getvalue(sem_full_normal, &nn);
    ...
    sem_wait(sem_mutex_priority);
    strcpy(shdata->priority_buf[shdata->priority_in], str);
    shdata->priority_in = (shdata->priority_in + 1) % ((K / 2) / (STR_LEN + 1));
    sem_post(sem_mutex_priority);
    sem_post(sem_full_priority);

    printf("[Manager] promoted NORMAL -> PRIORITY: %s\n", str);
    fflush(stdout);
  }
}
...
int main(void) {
  ...
  printf("Producers: %d  Consumers: %d  Buffer: %d  Priority: %d%%  "
         "Manager every %ds\n\n",
         N, M, K, PRIORITY_CHANCE, MANAGER_INTERVAL);
